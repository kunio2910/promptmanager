...
function PromptDetail({ prompt }: { prompt: PromptRecord | null }) {
  if (!prompt) return <div className="detail-column"><div className="panel detail-panel detail-empty"><div className="panel-heading"><h2>CHI TIẾT PROMPT</h2></div><p>Chưa có prompt để hiển thị. Hãy tải dữ liệu từ Google Sheet hoặc lưu prompt mới.</p></div></div>;
...
export default function Home() {
  ...
  const loadFromCloud = async () => {
    ...
    try {
      ...
      setSelectedPrompt(nextPrompts[0] || null);
